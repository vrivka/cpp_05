...
Form::Form( std::string const &name, std::string const &target, bool isSigned, int toSign, int toExec ) : Name(name), target(target), isSigned(isSigned), signGrade(toSign), execGrade(toExec) {
	if (toSign > 150)
